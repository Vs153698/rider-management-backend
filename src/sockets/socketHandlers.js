  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_direct_conversation', async (otherUserId) => {
      try {
        if (otherUserId === socket.userId) {
          return socket.emit('join_error', { message: 'Cannot start conversation with yourself' });
        }
    ...
    socket.on('join_ride', async (rideId) => {
      try {
        ...
        if (isMember === undefined) {
          ...
          isMember = ride && (ride.creator_id === socket.userId || 
    ...
    socket.on('join_group', async (groupId) => {
      try {
        ...
        if (isMember === undefined) {
          ...
          isMember = group && (group.admin_id === socket.userId || 
      ...
      try {
        ...
        if (chat_type === 'direct') {
          ...
          if (recipient_id === socket.userId) {
            return socket.emit('message_error', { message: 'Cannot send messages to yourself' });
          }
        ...
        if (chat_type === 'direct') {
          ...
          if (!connection) {
            ...
            try {
              connection = await UserConnection.findOrCreateConnection(socket.userId, recipient_id, socket.userId);
              if (connection) {
                await cacheSet(cacheKey, connection, 300);
                connectionCache.set(cacheKey, connection);
              }
              ...
              return socket.emit('message_error', { message: 'Failed to create connection' });
            }
          }
        ...
        if (!chat || !chat.id) {
          return socket.emit('message_error', { message: 'Failed to create message' });
        }
        ...
        if (error.name === 'SequelizeValidationError' && 
            error.errors.some(e => e.validatorKey === 'cannotConnectToSelf')) {
          socket.emit('message_error', { message: 'Cannot send messages to yourself' });
        } else if (error.name === 'SequelizeForeignKeyConstraintError') {
          socket.emit('message_error', { message: 'Invalid recipient or chat reference' });
        } else {
    ...
    socket.on('edit_message', async (data) => {
      try {
        ...
        const chat = await Chat.findByPk(message_id, {
          attributes: ['id', 'sender_id', 'chat_type', 'recipient_id', 'ride_id', 'group_id', 'is_deleted']
        });
      ...
      try {
        ...
        if (chat_type === 'direct' && sender_id) {
          if (sender_id === socket.userId) {
            return socket.emit('mark_read_error', { message: 'Cannot mark your own messages as read' });
          }
    ...
    socket.on('block_user', async (data) => {
      try {
        ...
        if (user_id === socket.userId) {
          return socket.emit('block_error', { message: 'Cannot block yourself' });
        }
    ...
    socket.on('share_location', async (data) => {
      try {
        ...
        if (chat_type === 'direct' && recipient_id === socket.userId) {
          return socket.emit('location_error', { message: 'Cannot share location with yourself' });
        }
    ...
    socket.on('start_conversation', async (data) => {
      try {
        ...
        let connection;
        try {
          connection = await UserConnection.findOrCreateConnection(socket.userId, user_id, socket.userId);
        } catch (error) {
          return socket.emit('conversation_error', { message: 'Failed to create connection' });
        }
  ...
  function processBatchedMessages() {
    ...
    batch.forEach(async (messageData) => {
      try {
        ...
        const messages = await Chat.bulkCreate(batch.map(data => ({
